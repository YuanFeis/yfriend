...
<link rel="stylesheet" href="${pageContext.request.contextPath }/css/cropbox.css" />
<link rel="stylesheet" href="${pageContext.request.contextPath }/css/bootstrap.min.css" />
<link rel="stylesheet" href="${pageContext.request.contextPath }/css/tanchu.css" />
...
<script src="${pageContext.request.contextPath }/js/easydialog.min.js"></script>
...
<body>
	...
	<div id="loader">
		...
		<div>
			<img id="logo" src="${pageContext.request.contextPath }/img/index.ico" />
			<div id="search">
				<input type="text" class="search" id="search_two"
					placeholder="搜索Yfriend用户" /><span
			...
			<div id="list">
				...
				<div id="listlist">
					...
					<hr>
					<div class="listd">
						<span class="glyphicon glyphicon-share" style="margin-right: 10px"></span>Yfriend广告
					</div>
					<div class="listd">
	...
	<div id="body">
		<div id="bodydetail">
			...
			<div class="buchongziliao">
				...
				<div style="margin-top:10px;margin-left:20px">
					...
					<div style="color:black;">让YF和你一起庆祝。</div>
		...
		<div id="second">
			<form action="${pageContext.request.contextPath }/tweettwo.do?method=newt" method="post" id="mytweet"
				enctype="multipart/form-data">
				<div id="send">
		...
		<div id="myModal" class="reveal-modal">
			...
			<div id="tanchu_touxiang">
				<img src="${pageContext.request.contextPath }/img/8_150610172935_9.jpg" />
			</div>
			<div id="nameandaite">
...
<script>
	...
	function siliao() {
	 	var url =  '/yfriend/message.do?method=hasnew';
		$.ajax({
			url : url,
			type : 'POST',
	...
	$(function() {
		if(${info.uabout == null} || ${info.udate ==null} || ${info.ulogo == "default_profile_normal.png"}){
			$("#tongzhi").html("欢迎来到YF的世界！请尽情展示你的freestyle！").fadeIn(300).delay(1000).fadeOut(300);
		}
		$("#touxiang").popover({
			content : "修改个人信息和登出",
			placement : 'bottom',
	...
	function catsession() {
		$.ajax({
			url : '/yfriend/user.do?method=catsession',
			type : 'POST',
			success : function(response, status) {
				if (response == "exit") {
	...
	function shishitongzhi() {
		$.ajax({
			url : '/yfriend/notify.do?method=noread',
			type : 'POST',
			success : function(response, status) {
...
<script>
	...
	function xiugaishengri(data) {
		$.ajax({
			url : '/yfriend/user.do?method=updatebrithday',
			type : 'POST',
			data : {
				date : data
			},
	...
	function gengxininfo(data) {
		$.ajax({
			url : '/yfriend/user.do?method=gengxininfo&neirong='+data,
			type : 'POST',
			success : function(response, status) {
				if(response == 'ok'){
					$(".buchongziliao:eq(3)").show();
	...
	function xiugaijianjie(data) {
		$.ajax({
			url : '/yfriend/user.do?method=updateabout',
			type : 'POST',
			data : {
				about : data
			},
...
<script>
	...
	var yfriend  =' ${pageContext.request.contextPath}';
	...
	function hasNew() {
		$.ajax({
			url : '/yfriend/tweettwo.do?method=gettweets&pagenum=0',
			type : 'POST',
			success : function(response, status) {
	...
	function jiaZai() {
		if( $("#back").css("display") == "none"){
			$('#jiazai img').fadeIn();
			var url = '/yfriend/tweettwo.do?method=gettweets&pagenum='+$(".tuiwen").length;
			$.ajax({
				url : url,
				type : 'POST',
				success : function(response, status) {
	...
	function gengXin() {
	 	var url =  '/yfriend/tweettwo.do?method=getnewtweets&td=top&nowid='+$(".tuiwen:eq(0)").find(".idid").val();
		$.ajax({
			url : url,
			type : 'POST',
			success : function(response, status) {
				if (response > 0) {	
					...
					window.document.title = "(" + response + ")Yfriend";
					$(".tishidian:eq(0)").show();
				}
			}
		});
	...
	function tianJia() {
	 	var url =  '/yfriend/tweettwo.do?method=getnewtweets&td=down&nowid='+$(".tuiwen:eq(0)").find(".idid").val();
		$.ajax({
			url : url,
			type : 'POST',
			success : function(response, status) {
	...
	for (var i = 0; i < tweets.length; i++) {
				 if(tweets[i].tcontent =="" && tweets[i].tpic == null && tweets[i].tzhaun == 0){
					...
					+ yfriend+'/img/'+tweets[i].uname+'/'+tweets[i].ulogo+ '\');background-size: 60px 60px;"></div><div class="sender_info"><div onmouseover="nameTianZiLiao(this)" onmouseout="removeZiLiaoKatwo(this)" class="sender_name" style="z-index:99;font-weight: bold;">'
						...
						if(tweets[i].utweets.tpic != null){
							...
							+yfriend+'/img/'+tweets[i].uname+'/'+tweets[i].utweets.tpic+'" >图片</a>';
						}
						html+='</div></div>'						 
						 }
						...
						+yfriend+'/img/'+tweets[i].uname+'/'+tweets[i].tpic+'" /></div>';
						}
	...
	function getreply(tid,page,aite){
			$.ajax({
			url : '/yfriend/reply.do?tid='+tid+'&page='+page,
			type : 'POST',
			success : function(response, status) {
	...
	function addReply(replys,aite){
		...
		for(var i = 0; i < replys.length; i++){
			...
			+ yfriend+'/img/'+replys[i].uname+'/'+replys[i].ulogo+'" /></div><div><div class="huifudename">'
...
function tweetsJs(){
	...
	$("#submit").click(function(){
		if($(".meiyou").css("display") == "block"){
			var url =  '/yfriend/tweettwo.do?method=getnewtweets&td=down&nowid=';
		}else{
			var url =  '/yfriend/tweettwo.do?method=getnewtweets&td=down&nowid='+$(".tuiwen:eq(0)").find(".idid").val();
		}
		$.ajax({
...
<script>
	...
	function shuaXinTuiJian() {
		$.ajax({
			url : '/yfriend/user.do?method=shuaxintuijian&suiji=' + Math.random(),
			type : 'POST',
			success : function(response, status) {
				var length = response.length;
	...
	function changeTuiJian(users) {
		...
		for (var i = 0; i < users.length; i++) {
			html += '<div class="tuijian_info"><div class="tuijian_touxiang" style="background: url(' + yfriend + '/img/'
			...
			$.ajax({
				...
				success : function(data) {
					if (data == "ok") {
						...
						+'<div style="width: 60px;height: 100%;float:left;"><img style="width:48px;height:48px;border-radius: 50px" src="'
						+ yfriend+'/img/'+"${user.uname}"+'/'+"${info.ulogo}"+'" /></div><div><div class="huifudename">'
	...
	function nameTianZiLiao(user) {
		var url = '/yfriend/user.do?method=getziliaoka&uid=' + $(user).parent().parent().find(".uuid").val() + '&suiji=' + Math.random();
	...
	function tianZiLiaoKa(user) {
		var url = '/yfriend/user.do?method=getziliaoka&uid=' + $(user).parent().find(".uuid").val() + '&suiji=' + Math.random();
		if($(".usersinfo").length < 1){
			$.ajax({
			url : url,
			type : 'POST',
	...
	function addZiLiaoKa(tweets, state) {
		...
		for (var i = 0; i < tweets.length; i++) {
			...
			if (tweets[i].ubg == null) {
				...
				html += 'style="background-color: ' + tweets[i].ucolor + ';background: url(' + yfriend + '/img/' + tweets[i].uname + "/" + tweets[i].ubg + ');';
			}
			html += 'background-size: 300px 100px;"></div><div style="width:300px;height:200px;"><div class="waiyuan">' +
				'<div class="yuantouxiang" style="background: url(' + yfriend + '/img/' + tweets[i].uname + "/" + tweets[i].ulogo + ');background-size: 70px 70px;"></div></div>'
	...
	$(function() {
		...
		$('#btnCrop').on('click', function() {
			...
			var url = '/yfriend/user.do?method=xiugaitouxiang';
			$.ajax({
				url : url,
				data : {
					"touxiang" : img
	...
	$("#search_two").bind("input change", function() {
		...
		var url = '/yfriend/user.do?method=chayonghu';
		if ($(this).val().trim() != "") {
			$.ajax({
				url : url,
				data : {
				...
				success : function(response, status) {
					...
					for (var i = 0; i < users.length; i++) {
						html += '<div class="souneirong" onclick="chaxunuser(0,this)"><div class="soutouxiang"><img src="'
							+ yfriend + '/img/' + users[i].uname + "/" + users[i].ulogo + '" /></div><div class="soumingzi">'
