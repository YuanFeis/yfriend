...
<head>
<meta charset="utf-8">
<title>${who.urealname }(@${who.uaite })&nbsp;|&nbsp;Yfriend</title>
...
<script>
	...
	function jiaZai() {
		if( $("#back").css("display") == "none"){
			$('#jiazai img').fadeIn();
			var url = '/yfriend/tweettwo.do?method=gettweets&num=one&who=other&pagenum='+page;
			$.ajax({
				url : url,
				// '/yfriend/tweettwo.do?method=getnewtweets&td=down&nowid='+$(".tuiwen:eq(0)").find(".idid").val(),
				type : 'POST',
				success : function(response, status) {
					$("#jiazai img").fadeOut();
	...
	function hasNew() {
		$.ajax({
			url : '/yfriend/tweettwo.do?method=gettweets&num=one&who=other&pagenum='+page,
			type : 'POST',
			success : function(response, status) {
