...
<link href="${pageContext.request.contextPath }/css/reveal-it.css" rel="stylesheet">
<script src="${pageContext.request.contextPath }/js/jquery.animate-colors-min.js"></script>
<script src="${pageContext.request.contextPath }/js/reveal-it.js"></script>
<style>
* {
	margin: 0;
	padding: 0;
			...
			if (xmlHttp.readyState == 4) { //如果等于4，说明服务器响应成功了
				...
				if (xmlHttp.status == 200) { //如果服务器响应码是200时
					...
					if (data == "yes" && txt != "" && txt != null && regex.test(txt)) {
						...
						$(".check").html("<img src='/yfriend/img/error.png'/><span style='color:tomato;font-size:14px'>该用户名已被占用。</span>");
					} else {
						$(".check").html("<img src='/yfriend/img/error.png'/><span style='color:tomato;font-size:14px'>只支持英文字母和数字。</span>");
					}
				}
			}
		}
	...
	$(function() {
		var slider = new SliderUnlock("#slider", {
			...
			window.setInterval(function() {
				window.location.href = "/yfriend/user.do?method=signup&aite=" + $("#txt").val();
			}, 3000);
		});
		slider.init();
	});
