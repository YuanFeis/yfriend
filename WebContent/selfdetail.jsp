...
<head>
<meta charset="utf-8">
<title>${user.urealname }(@${user.uaite })&nbsp;|&nbsp;Yfriend</title>
...
<script type="text/javascript">
	...
	function baocunxiugai() {
		...
		$.ajax({
			url : '/yfriend/user.do?method=xiugaiziliao',
			type : 'POST',
			data : {
				urealname : realname,
				uabout : about,
...
<script>
	...
	function jiaZai() {
		if( $("#back").css("display") == "none"){
			$('#jiazai img').fadeIn();
			var url = '/yfriend/tweettwo.do?method=gettweets&num=one&pagenum='+page;
			$.ajax({
				url : url,
				type : 'POST',
				success : function(response, status) {
	...
	function hasNew() {
		$.ajax({
			url : '/yfriend/tweettwo.do?method=gettweets&num=one&pagenum='+page,
			type : 'POST',
			success : function(response, status) {
	...
	for (var i = 0; i < tweets.length; i++) {
	 if(tweets[i].tcontent =="" && tweets[i].tpic == null){
				 	...
				 	+yfriend+'/img/'+tweets[i].uname+"/"+tweets[i].ulogo+ '\');background-size: 60px 60px"></div><div class="sender_info"><div class="sender_name" style="font-weight: bold;">'
...
<script>
	...
	function checkaite() {
		...
		xmlHttp.open("GET", "/yfriend/user.do?method=checkaite&aite=" + txt, false);
		xmlHttp.send(null);
	}
</script>
</html>