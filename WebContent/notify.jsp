...
<script>
	...
	function shishitongzhi() {
		$.ajax({
			...
			success : function(response, status) {
				if (response > 0) {
					window.document.title = "(" + response + ")Yfriend / 通知";
				}
			}
		});
	}
	...
	function chaxuntongzhi() {
		...
		$.ajax({
			url : '/yfriend/notify.do?method=all&page=' + page,
			type : 'POST',
			success : function(response, status) {
		...
		for (var i = 0; i < notification.length; i++) {
			if (notification[i].ntype == 1) {
				html += '<div class="notifications"><div	style="margin-left: 55px;padding-top: 20px;height: 26px;line-height: 26px">	<span class=" glyphicon glyphicon-user"'
					...
					+ ' src="' + yfriend + '/img/' + notification[i].usersall.uname + '/' + notification[i].usersall.ulogo + '" /></div><div style="margin-left: 85px;margin-top: 20px;color: black;padding-bottom: 20px">'
			...
			if (notification[i].ntype == 2) {
				...
				if (notification[i].utweets.tpic != null) {
					html += '<a style="margin-left:10px;" target="view_window" href="' + yfriend + '/img/' + notification[i].utweets.uname + '/' + notification[i].utweets.tpic + '">图片</a>';
				}
			...
			if (notification[i].ntype == 3) {
				...
				+ yfriend + '/img/' + notification[i].utweets.uname + '/' + notification[i].utweets.ulogo + '" />	</div><div style="margin-left: 85px;margin-top: 20px;color: black;padding-bottom: 20px">'
				+ '<span class="guanzhurenming" style="font-weight: bold;margin-right: 5px;cursor: pointer;" onclick=chakanwho(this)>'
				...
				if (notification[i].utweetstwo.tpic != null) {
					html += '<a style="margin-left:10px;" target="view_window" href="' + yfriend + '/img/' + notification[i].utweetstwo.uname + '/' + notification[i].utweetstwo.tpic + '">图片</a>';
				}
				html += '</div></div></div></div>';
			}
			if (notification[i].ntype == 4) {
				...
				+ '<img style="width: 31px;height: 31px;border-radius: 50px;background-color: white;margin-top: -10px" src="'
				+ yfriend + '/img/' + notification[i].usersall.uname + '/' + notification[i].usersall.ulogo + '" />	</div><div style="margin-left: 85px;margin-top: 20px;color: black;padding-bottom: 20px">'
				+ '<span class="guanzhurenming" style="font-weight: bold;margin-right: 5px;cursor: pointer;" onclick=chakanwho(this)>'
				+ notification[i].usersall.urealname + '</span> <input type="hidden" class="uuid" value="'
